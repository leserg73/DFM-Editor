  ...
  begin
    ...
    begin
      ...
      begin
        ...
        c2name:= getCollectionClass(cname,GetPropertyName(tn));
        C1:=GetClassDef(c2name);
        if assigned(C1) then
...
begin
  ...
  try
    ...
    form_dfmPreview.Realign;//avoid problems with window-state=wsMaximized; must be after show
    loading:=false;
  finally
    OutStream.Free;
    InStream.Free;
